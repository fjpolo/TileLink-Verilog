...
function usage {
    ...
    echo "usage: $programname [-h] [-c] [-i infile] [-o outfile] [-s scriptfile] [-d designfile]"
...

{
    ...
    done < "$input_file"  # Quote $input_file

    echo "clk2fflogic"  # Correct placement

    echo "$write_cmd"
} > "$tmp_script"

yosys -ql ${script_file%%.ys}.log -s "$tmp_script"  # Add -s option
