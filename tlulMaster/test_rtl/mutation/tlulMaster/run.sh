    ...
    cp ${PWD}/../../../rtl/tlulMaster.v .

    # Append `define MCY after `timescale 1ps/1ps to tlulMaster.v using awk
    awk '1;/`timescale 1ps\/1ps/{print "`define MCY"}' tlulMaster.v > template_temp.v
    mv template_temp.v tlulMaster.v

    #replace the orginal testbench file with the temp file.
    mv template_temp.v tlulMaster.v

    # Copy tlulMaster here
    cp ${PWD}/../../simulation/icarus/tlulMaster/testbench.v .
